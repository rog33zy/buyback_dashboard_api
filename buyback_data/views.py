...
from .serializers import (
    BuybackDataSerializer,
    BuybackDataSerializerTwo,
    BuybackDataSerializerThree,
)
        ...
        if category_param is None:
            queryset = (
                ...
                .annotate(
                    ...
                    total_amount_paid_zmw=Sum("paid_amount_zmw"),
                    total_amount_paid_usd=Sum("paid_amount_usd"),
...
def post_category_view(request):
    if request.method == "POST":
        ...
        if buyback_entry_serializer.is_valid():
            ...
            return JsonResponse(
                buyback_entry_serializer.data, status=status.HTTP_201_CREATED
            )
        return JsonResponse(
            buyback_entry_serializer.errors, status=status.HTTP_400_BAD_REQUEST
        )
...
def edit_category_view(request, pk):
    ...
    if request.method == "PATCH":
        buyback_entry_data = JSONParser().parse(request)
        buyback_entry_serializer = BuybackDataSerializer(
            detailed_buyback_entry, data=buyback_entry_data, partial=True
        )

        if buyback_entry_serializer.is_valid():
            buyback_entry_serializer.save()
            return JsonResponse(
                buyback_entry_serializer.data, status=status.HTTP_200_OK
            )
